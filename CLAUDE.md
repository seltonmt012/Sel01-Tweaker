...
.\dist\Sel01Tweaker.ps1 -Profile Gaming  # default profile
.\dist\Sel01Tweaker.ps1 -Profile Clean
.\dist\Sel01Tweaker.ps1 -DryRun          # log intended changes, write nothing
...
$Global:Sel01Tweaker.Profile = 'Gaming'
...
**Orchestrator-hybrid.** `src/Sel01Tweaker.ps1` (entry) self-elevates, initializes
`$Global:Sel01Tweaker` state, optionally makes a restore point, then runs 7 module
...
`$Global:Sel01Tweaker.Profile`. Gaming keeps Game Mode + HAGS on and trims gently;
...
- After editing anything in `src/`, **rebuild** (`.\build.ps1`) and re-run
  `run-checks.ps1`. Runtime logs/backups live in `%ProgramData%\Sel01Tweaker\`.
