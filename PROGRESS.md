...
- [x] **v1.2.0** - FiveM cache cleaner (whitelist subfolders, skip game/citizen, skip if running), Fast Startup off (HiberbootEnabled=0), opt-in `-TimerFix` (Win11 GlobalTimerResolutionRequests) + `-MsiMode` (GPU MSI, auto-detects GPU via PnP). Researched via 2 fan-out subagents (FiveM client + gaming/latency); excluded MouseDataQueueSize/global-FSO/SwapEffectUpgrade/core-unparking/DXVK as placebo or harmful. Also fixed `-SkipClean` not being passed through.
- [x] **v1.1.2** - humanizer/docs pass: removed em-dashes + decorative emojis across docs, rewrote ANLEITUNG.md in plain language, synced beginner guide + README to the new 3-choice menu. No behaviour change.
...
- [x] **v1.9.0 - OpenTweak review** (github.com/SwisserDev/OpenTweak, MIT, C#/WPF - reviewed its 24 tweak docs against ours; ~85% already covered, no code copied). Adopted the three real gaps:
  - **Module 19-ShaderCache** (opt-in `-ShaderClean` + advanced-menu entry [4]): clears NVIDIA `DXCache`/`GLCache`/`NV_Cache`, AMD `DxCache`/`DxcCache`/`GLCache`/`VkCache`, Intel `ShaderCache`, `D3DSCache`, `Microsoft\DirectX Shader Cache`. Vendor gating is implicit (only existing dirs), keeps the dirs themselves, skips locked files, warns when a game is running. Opt-in on purpose: the next game start recompiles shaders once (GTA V 1-2 min). Not revertable (cleaner class, like module 12). Verified DryRun on this box: 3 caches, ~1.4 GB.
  - **Module 11-Power re-gated**: `PowerThrottlingOff` now also runs on a **laptop on mains** (that is where Windows throttles hardest - OpenTweak's point); the powercfg device block (USB suspend/PCIe ASPM/disk) and `-MsiMode` stay desktop-only; **battery still skips everything**.
  - **Module 06-Gaming + GTA V**: `Get-Sel01GtaVExePaths` finds GTA5.exe / GTA5_Enhanced.exe via Rockstar registry, Steam `libraryfolders.vdf` and Epic manifests, then applies the same per-app FSO-off + `GpuPreference=2` that module 08 applies to the FiveM exes (module 08 is FiveM-gated, so a plain GTA V install never got them). Verified: found the Steam install on E:.
  - **README "Honest effectiveness" table** (✅/⚠️/❌ per tweak + the deliberately-not-shipped list), in the spirit of OpenTweak's ratings. NOTICE.md credits it as a documentation reference.
  - Rejected from OpenTweak: **VBS/HVCI disable** (violates the never-weaken-security invariant, breaks anti-cheats), their **timer resolution** (`NtSetTimerResolution` only lasts while their GUI runs - our `GlobalTimerResolutionRequests` is the persistent equivalent), **CPU unparking** (they call it placebo themselves).
...
- [x] **v1.10.0 - RAM cleaner rollback** (bug fix, not a feature - this shipped to real users and froze a real machine). Up to v1.9.0 module 07 ran unconditionally and installed the scheduled task `Sel01Tweaker-RamCleaner`, firing at boot +3 min and then hourly as SYSTEM, calling `EmptyWorkingSet()` on every process, purging the standby list and flushing the system file cache. That evicts the machine's entire live working set to the pagefile once an hour; everything the user touches next has to be faulted back in, which is the recurring multi-minute freeze that was reported. It leaves no trace in the event log (Resource-Exhaustion-Detector only fires near the commit limit, which is never reached), and it is far worse when the pagefile sits on an HDD. We should never have shipped a periodic task that mutates system state.
  - **Task registration removed entirely**, and `EmptyWorkingSet` + `SetSystemFileCacheSize` are gone from the code - not gated, deleted. What is left is `NtSetSystemInformation` commands 3 (flush modified page list) and 4 (purge standby list).
  - **The purge is now opt-in and one-shot** via the new `-RamClean` switch (wired through `param()`, the elevation `$argline`, the state stash and `Start-Sel01Tweaker`), same shape as `-ShaderClean`/module 19. Off by default; it also skips when a reboot is already pending, and it **refuses to run when the pagefile is on an HDD** (`Test-Sel01PagefileOnHdd` via `Win32_PageFileUsage` → `Get-Partition` → `Get-PhysicalDisk`), logging why.
  - **Upgrading users are repaired automatically**: every run calls `Remove-LegacyRamCleanerTask`, which unregisters the hourly task and deletes the `Sel01Tweaker-RamClean.ps1` helper script left in `%ProgramData%\Sel01Tweaker\`, honours DryRun, and records it in the change summary. Nobody has to know the task was ever there.
  - `-NoRamTask` still binds so old command lines don't break, but it is a documented no-op. `RamTaskName` is no longer set, so the backup JSON's `RamTask` field is always null on new runs - `Invoke-Revert` only uses it when reverting from a pre-v1.10.0 backup.
  - Docs corrected in the same pass: the README honest-effectiveness row is downgraded to ❌ with the actual mechanism spelled out, periodic memory cleaning is added to the deliberately-not-shipped list, ANLEITUNG.md no longer claims the tool "cleans up RAM", NOTICE.md lists only the API still reimplemented, and CLAUDE.md gained a hard invariant against background tasks and scheduled working-set eviction. Verified by reading the rewritten module against every doc claim; no measurements were re-run.
