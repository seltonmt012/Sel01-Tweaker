# Pester tests for Twerk (non-destructive).
# Run:  Invoke-Pester -Path .\tests\Twerk.Tests.ps1
# Registry tests use a throwaway HKCU:\Software\TwerkTest key and clean up.
...
AfterAll {
    if (Test-Path 'HKCU:\Software\TwerkTest') {
        Remove-Item 'HKCU:\Software\TwerkTest' -Recurse -Force -ErrorAction SilentlyContinue
    }
}
...
Describe 'Set-Reg snapshot + typed write' {
    ...
    It 'writes a DWORD and records it did not exist before' {
        ...
        $snap = $Global:Twerk.Backup | Where-Object Name -eq 'Num'
    ...
    It 'snapshots the ORIGINAL value on repeated writes' {
        ...
        ($Global:Twerk.Backup | Where-Object Name -eq 'Num').Count | Should -Be 1
...
Describe 'Set-Reg DryRun' {
    It 'records a snapshot but writes nothing' {
        ...
        ($Global:Twerk.Backup | Where-Object Name -eq 'DryVal') | Should -Not -BeNullOrEmpty
        $Global:Twerk.DryRun = $false
    }
}
...
Describe 'Revert round-trip' {
    It 'restores a pre-existing value and removes an added value' {
        ...
        $Global:Twerk.BackupFile = Join-Path $env:TEMP 'backup-test.json'
        $Global:Twerk.Stamp = 'test'
        ([ordered]@{ Profile='Gaming'; Created='test'; PowerSchemeGuid=$null; RamTask=$null; Registry=$Global:Twerk.Backup } |
            ConvertTo-Json -Depth 6) | Set-Content $Global:Twerk.BackupFile -Encoding UTF8

        Invoke-Revert -BackupPath $Global:Twerk.BackupFile
        ...
        Remove-Item $Global:Twerk.BackupFile -Force -ErrorAction SilentlyContinue
    }
}
